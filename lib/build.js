/**
 * Build and optionally minify the source files from `app` directory.
 * opts:
 *  -q [quiet]
 *  -m [minify]
 ...
 *
 * todo:
 *   build process needs an overhaul, particularly the source transformation
 *   should use AST instead of string processing
 *   1) refactor escapeStrings, preProcess, stripSource
 *   2) deprecate appendLines, --mangle
 *   3) replace uglify with esprima equivalent
 */
...
(function() {
  ...
  var utils = require('../modules/drift-utils/utils');
  ...
  // a mutable object that allows us to collect options from source files (docblock directives)
  var sourceOptions = {};

  //files beginning with these chars are ignored
  var EXCLUDE = {'_': 1, '.': 1, '!': 1}; // eslint-disable-line quote-props
  ...
  var config = utils.readJSON(join(projectPath, 'build-conf.json'));
  var pkgConfig = utils.readJSON(join(projectPath, 'package.json'));
  ...
  if (opts.platform == 'apache') {
    ...
    opts._load = [
      //load polyfills
      'app/system/support/es6-shim.js',
  ...
  if (opts.platform == 'iis') {
    ...
    opts._load = [
      //load polyfills
      'app/system/support/es5-shim.js',
      'app/system/support/es6-shim.js',
      'app/system/support/json2.js',
      //load framework core (instantiates `app` and `define`)
      'app/system/core.js',
      //load config
      'app/system/config',
  ...
  if (config.compileViews) {
    ...
    opts._head.push('global.compiledViews = ' + escapeStrings(compileViews) + ';');
  }
  ...
  if (sourceOptions.includeES6Polyfills) {
    // todo: add ES6 Polyfills
  }
  ...
  function loadFile(path) {
    //skip this file if it matches any path in exclude list
    var skip = false;
    ...
    if (skip) return;
    //load file and apply source transformations
    if (!opts.q) console.log('load file', path);
    var fullpath = join(projectPath, path);
    var filedata = fs.readFileSync(fullpath, 'utf8');
    filedata = utils.transformSourceFile(filedata, fullpath, {pkgConfig: pkgConfig}, sourceOptions);
    //escape unicode inside strings (and some special html sequences)
    filedata = escapeStrings(filedata);
